...
- **🤖 自动化策略**：
  ...
  - **定时回测**：定时调度自动执行策略历史回测，持续更新回测快照。
- **❤️ 智能自选管理**：灵活的分组管理，支持快速添加股票（代码智能识别补全），并可跨模块联动筛选。
- **🔔 价格智能通知**：自定义价格监控规则，定时检查实时行情，触发阈值时自动发送邮件通知用户。
- **📝 文章笔记管理**：
...
![股票详情.png](screenshots/股票详情.png)


### 板块数据
![板块数据.png](screenshots/板块数据.png)
![板块详情.png](screenshots/板块详情.png)
